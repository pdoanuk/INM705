...
# --- Configuration ---
MODEL_V1 = "ViTAutoencoder"
MODEL_V2 = "VitDecoderExp"
MODEL_V3 = "ViTAD_Fusion"
MODEL_V4 = "ViTAD_Fusion_v2"
...
def get_model(args: argparse.Namespace, device: torch.device) -> nn.Module:
    ...
    if args.model.lower() == MODEL_V3.lower():
        model = load_default_model().to(device)
    elif args.model.lower() == MODEL_V4.lower():
        model = load_default_vitad_model().to(device)
    elif args.model == MODEL_V2.lower():
        model = VitDecoderExp().to(device)
    elif args.model == MODEL_V1.lower():
...
def train_epoch(
    ...
    for x, y, mask, cls_name in pbar:
        ...
        pbar.set_postfix(loss=f"{batch_loss:.6f} :: {loss:.6f}")
...
def run_experiment(
    ...
    try:
        ...
        logger.info(
            f"Approx Train samples: {len(train_loader.dataset)}, Approx Val samples: {len(val_loader.dataset)}, Test samples: {len(test_loader.dataset)}")
    except Exception as e:
    ...
    for epoch in range(1, class_args.epochs + 1):
        ...
        if epoch % class_args.val_epochs == 0 or epoch == class_args.epochs:  # Validate on schedule or last epoch
            ...
            logger.info(
                f"Epoch {epoch}/{class_args.epochs} => Class: {class_name}, Train Loss: {train_loss:.6f}, Val Loss: {val_loss:.6f}")
...
def run_experiment_all(
    ...
    try:
        ...
        logger.info(
            f"Total Train samples: {len(train_loader.dataset)}, Total Val samples: {len(val_loader.dataset)}, Total Test samples: {len(test_loader.dataset)}")
    except Exception as e:
...
def main():
    ...
    if run_mode.lower() == "sweep":
        args.project =  "INM705 - EXP - SWEEP"
    else:
        args.project = "INM705 - EXP"
    ...
    if wandb_mode != "disabled":
        try:
            ...
            run_name = f"Run_mode_{run_mode}_{args.model}_{run_identifier}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
            wandb_run = wandb.init(
                project=args.project,
                name=run_name,
                tags=args.tags + [args.model, args.obj if args.obj else 'N/A'] + [run_mode],
                notes=f"{args.notes} _ model {args.model} loss_func {args.loss_func} _ run_mode {run_mode}",
    ...
    if run_mode.lower() in ["full", "sweep"]:
        logger.info(f"Perform {run_mode} run.")
        if args.obj == 'all':  # Run training for all classes separately
            ...
            try:
                ...
                logger.error(f"An unexpected error occurred during the 'single_class' execution for {class_name}: {e}",
                             exc_info=True)
        else:
